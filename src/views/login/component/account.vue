<template>
    <el-form size="large" class="login-content-form">
        <el-form-item class="login-animation1">
            <el-input type="text" placeholder="用户名" v-model="ruleForm.username" clearable autocomplete="off">
                <template #prefix>
...
<script setup lang="ts">
    ...
    const state = reactive({
        ...
        ruleForm: {
            ...
            grant_type: "password",
            client_id: "pbd",
            client_secret: "cnbisoft"
        },
        loading: {
            signIn: false
        }
    });
    const {isShowPassword, ruleForm, loading} = toRefs(state);
    // 时间获取
    ...
    const onSignIn = async () => {
        ...
        const loginResult = await login(state.ruleForm);
        if (loginResult.code !== 200) {
            proxy.$baseMessage(loginResult.msg, "error");
        ...
        // 存储 token 到浏览器缓存
        Session.set("token", loginResult.data.access_token);
        // 存储用户信息到浏览器缓存
    ...
    const signInSuccess = () => {
        ...
        proxy.$baseMessage(`${currentTimeInfo}，${signInText}`, "success");
    };
</script>
