<template>
    <div>
        ...
        <WzScientificEdit ref="editRef" @fetch-data="fetchData"/>
    </div>
</template>
<script lang="ts">
    export default {
        name: "WzScientificIndex",
        metaDataDict: []
    };
</script>
<script setup lang="ts">
    ...
    import {IWzScientific} from "/@/views/wz/wzScientific/interface/IWzScientific";
    import WzScientificEdit from "/@/views/wz/wzScientific/component/edit.vue";
    import {find_list_by_page, delete_id} from "/@/api/wz/wzScientific";
    ...
    /** 打开编辑弹窗**/
    const openEditDialog = (row: IWzScientific) => {
        editRef.value.openDialog(row.nid);
    };
    /**删除当前行**/
    const handleDelete = (row: IWzScientific) => {
