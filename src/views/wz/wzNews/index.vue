<template>
    <div>
        ...
        <WzNewsEdit ref="editRef" @fetch-data="fetchData"/>
    </div>
</template>
<script lang="ts">
    export default {
...
<script setup lang="ts">
    ...
    import {IWzNews} from "/@/views/wz/wzNews/interface/IWzNews";
    import WzNewsEdit from "/@/views/wz/wzNews/component/edit.vue";
    import {find_list_by_page, delete_id} from "/@/api/wz/wzNews";
    ...
    /** 打开编辑弹窗**/
    const openEditDialog = (row: IWzNews) => {
        editRef.value.openDialog(row.nid);
    };
    /**删除当前行**/
    const handleDelete = (row: IWzNews) => {
